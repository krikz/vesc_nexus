...
### Процедура калибровки (один раз для каждого колеса)
...
5. **Записать в конфиг**:
   ```yaml
   wheel_max_rps: [15.0, 15.0, 15.0, 15.0]  # замерить для каждого колеса!
   ```

6. **(Опционально)** Проверить обратное направление (duty = -100%)
