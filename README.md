...
При управлении через **duty cycle** (ШИМ) нужно знать соотношение между duty и реальной скоростью колеса. Без этого знания невозможно корректно преобразовать команду скорости от контроллера в duty cycle.

### 🚀 Автоматическая калибровка (рекомендуется)

Используйте скрипты из папки `tools/`:
...
1. При **duty = 100%** колесо крутится с максимальной скоростью
2. Зависимость предполагается **линейной**: duty 50% → половина оборотов
3. Формула: `duty = target_speed / max_speed`

### Ручная калибровка

Если нужно откалибровать вручную:

1. **Поставить робота на подставку** (колёса должны свободно крутиться)

2. **Подать duty = 100%** на колесо через VESC Tool или команду:
   ```bash
   # Через CAN (пример для VESC ID 49)
   cansend can0 031#00186A00  # duty = 100000 (100%)
   ```

3. **Замерить RPM** из телеметрии VESC:
   ```bash
   ros2 topic echo /sensors/motor_state/front_left --once
   # Смотрим поле speed_rpm
   ```

4. **Рассчитать max_rps**:
   ```
   wheel_max_rps = RPM / 60
   ```
   Например: 900 RPM → 15.0 об/сек
...
Допустим при калибровке получили:
- duty = 100% → RPM = 900 → **max_rps = 15.0** об/сек
- wheel_radius = 0.115 м

Тогда максимальная линейная скорость:
```
max_speed = 2π × max_rps × wheel_radius
          = 2π × 15.0 × 0.115 = 10.84 м/с
```

И для целевой скорости 1 м/с:
```
duty = 1.0 / 10.84 ≈ 0.092 (9.2%)
```

### Рекомендации

- Калибруйте при типичном уровне заряда батареи
- Если колёса показывают разные RPM — укажите индивидуальные значения
- Периодически проверяйте калибровку (износ, изменение условий)
...
├── tools/                        # 🆕 Скрипты калибровки
│   ├── README.md
│   ├── calibrate_max_rpm.py      # Автокалибровка max RPM
│   └── linearity_test.py         # Тест линейности duty→RPM
└── Dockerfile
```
